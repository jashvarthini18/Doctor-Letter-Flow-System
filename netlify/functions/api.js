...
app.post(["/drafts", "/api/drafts"], async (req, res) => {
  try {
    ...
    if (!doctor_id || !template_id || !content_html) {
      return res.status(400).json({ message: "doctor_id, template_id and content_html are required" });
    }

    const existingDraft = await pool.query(
      `SELECT * FROM drafts WHERE doctor_id = $1 AND template_id = $2`,
      [doctor_id, template_id]
    );

    if (existingDraft.rows.length > 0) {
      const updatedDraft = await pool.query(
        `UPDATE drafts SET content_html = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2 RETURNING *`,
        [content_html, existingDraft.rows[0].id]
      );
      return res.json({ message: "Draft updated successfully", draft: updatedDraft.rows[0] });
    }

    const newDraft = await pool.query(
      `INSERT INTO drafts (doctor_id, template_id, content_html) VALUES ($1, $2, $3) RETURNING *`,
      [doctor_id, template_id, content_html]
    );

    return res.status(201).json({ message: "Draft created successfully", draft: newDraft.rows[0] });
  } catch (error) {
    console.error(error);
...
app.get(["/drafts", "/api/drafts"], async (req, res) => {
  try {
    ...
    if (!doctor_id || !template_id) {
      return res.status(400).json({ message: "doctor_id and template_id are required" });
    }
...
app.post(["/letters/generate", "/api/letters/generate"], async (req, res) => {
  try {
    ...
    if (!doctor_id || !content_html) {
      return res.status(400).json({ message: "doctor_id and content_html are required" });
    }
    ...
    const safeLetterType = (letter_type || "Other").replace(/[^a-zA-Z0-9-_]/g, "_");

    const s3Key = `doctors/${doctor_id}/${safeLetterType}/${year}/${month}/letter-${uniqueId}.pdf`;
    ...
    const result = await pool.query(
      `INSERT INTO letters (doctor_id, template_id, patient_name, letter_type, content_html, s3_key)
       VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`,
      [doctor_id, template_id || null, patient_name || null, letter_type || null, content_html, s3Key]
    );
...
app.get(["/letters", "/api/letters"], async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `SELECT id, doctor_id, template_id, patient_name, letter_type, content_html, s3_key, created_at, updated_at
       FROM letters WHERE doctor_id = $1 ORDER BY created_at DESC`,
      [doctor_id]
    );
    res.json({ letters: result.rows });
  } catch (error) {
    console.error("HISTORY ERROR:", error);
    res.status(500).json({ message: "Failed to fetch letter history", error: error.message });
  }
});
...
app.get(["/letters/:id/download", "/api/letters/:id/download"], async (req, res) => {
  try {
    ...
    const downloadUrl = await getPDFDownloadUrl(result.rows[0].s3_key);
...
app.get(["/doctors/:id/profile", "/api/doctors/:id/profile"], async (req, res) => {
  try {
    const { id } = req.params;
    const result = await pool.query(
      `SELECT id, mobile_number, name, clinic_name, clinic_address, logo_url, signature_url
    ...
    if (doctor.logo_url) logoSignedUrl = await getSignedFileUrl(doctor.logo_url);
    if (doctor.signature_url) signatureSignedUrl = await getSignedFileUrl(doctor.signature_url);

    res.json({
      doctor: { ...doctor, logo_url: logoSignedUrl, signature_url: signatureSignedUrl },
    });
  } catch (error) {
...
app.delete(["/letters/:id", "/api/letters/:id"], async (req, res) => {
  try {
    ...
    res.json({ message: "Letter and PDF deleted successfully" });
  } catch (error) {
